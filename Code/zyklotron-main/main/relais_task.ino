...
const int hallPins[4] = { 4, 16, 17, 5 };
...
void IRAM_ATTR relaisTrigger(uint8_t index) {
  ...
  // 0–100 % → 0–30 ms
  int relaisZeitMs = map(tempoLok, 0, 100, 0, 30);
  relaisZeitMs = constrain(relaisZeitMs, 0, 30);
...
void relaisTask(void *pvParameters) {
  ...
  // ---------- INTERRUPTS ----------
  attachInterrupt(digitalPinToInterrupt(hallPins[0]), hallISR0, RISING);
  attachInterrupt(digitalPinToInterrupt(hallPins[1]), hallISR1, RISING);
  attachInterrupt(digitalPinToInterrupt(hallPins[2]), hallISR2, RISING);
  attachInterrupt(digitalPinToInterrupt(hallPins[3]), hallISR3, RISING);
