...
#define SDA_PIN 22  // OLED SDA
#define SCL_PIN 23  // OLED SCK / SCL
...
// ---------- SEITEN DEFINIEREN ----------
Seite seiten[] = {
  { "Helligkeit Rot", LAYOUT_BALKEN, 75 },
  { "Helligkeit Blau", LAYOUT_BALKEN, 75 },
  { "Helligkeit Grün", LAYOUT_BALKEN, 75 },
  { "Tempo", LAYOUT_GROSSZAHL, 100 },
...
void displayTask(void *pvParameters) {
  ...
  while (true) {
    ...
    if (pos != letzteEncoderPos) {
      ...
      if (!editModus) {
        ...
        if (strcmp(seiten[aktuelleSeite].titel, "Helligkeit Rot") == 0) {
          redValue = seiten[aktuelleSeite].wert;
        }

        if (strcmp(seiten[aktuelleSeite].titel, "Helligkeit Blau") == 0) {
          blueValue = seiten[aktuelleSeite].wert;
        }

        if (strcmp(seiten[aktuelleSeite].titel, "Helligkeit Grün") == 0) {
          greenValue = seiten[aktuelleSeite].wert;
        }
      }
  ...
  analogWrite(redPin, 255 - map(redValue, 0, 100, 0, 255));
  analogWrite(greenPin, 255 - map(greenValue, 0, 100, 0, 255));
  analogWrite(bluePin, 255 - map(blueValue, 0, 100, 0, 255));
}
...
void zeichneAktuelleSeite() {
  Serial.println(seiten[aktuelleSeite].titel);
  display.clearDisplay();
